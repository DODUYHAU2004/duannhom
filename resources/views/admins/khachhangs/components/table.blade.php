    ...
    <tbody>
        ...
        @if($value->trang_thai === 1)
            <tr>
                ...
                <td class="text-center"><a style="margin-right: 2px" href="{{route('khachhang.edit', $value->id)}}"><button
                    class="btn btn-success"><i class="fa fa-edit"></i></button></a>
                    <form action="{{route('khachhang.destroy', $value->id)}}" method="post" style="display: inline-block" onsubmit="confirm('Bạn có chắc chắn muốn chuyển vào thùng rác không ?')">
                        @csrf
                        @method('delete')
